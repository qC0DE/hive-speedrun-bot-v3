    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if resp.status_code != 200:
                logger.error(f"API Error! Status: {resp.status_code}, Response: {resp.text}")
...
def resolve_division(division_key: str, force: bool = False) -> ResolvedDivision:
    if not force and division_key in _resolved_cache:
        return _resolved_cache[division_key]
...
def fetch_leaderboard(division_key: str, max_entries: int = 100) -> list[PlayerEntry]:
